...
class MAFCond(nn.Module):
    ...
    """
    def __init__(self, dimx0, dimx, base_network=FCNN, config=None):
        super().__init__()
        self.dimx0 = dimx0
        self.dimx  = dimx
        self.layers = nn.ModuleList()
        self.layers += [base_network(dimx0, 2, config.net_spec["nodes"], config.net_spec["layer"], config.net_spec["act"])]
        for i in range(1, dimx0):
            self.layers += [base_network(i+dimx0, 2, config.net_spec["nodes"], config.net_spec["layer"], config.net_spec["act"])]

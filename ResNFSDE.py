...
class NFSSDE(nn.Module):
	...
	def train(self, train_data, model_path, hist_path, Monitor, DatVes, predt_path):
		...
		try:
			ThLikelihoodData = self.ThLikelihood(self.eqn_config.eqn_name,train_data[:,:self.dim],train_data[:,self.dim:],self.eqn_config.Delta)
		except:
	...
	def ThLikelihood(self,name,x0,x1,Delta):
		if self.dim==1:
			...
			a0,b0 = m*Delta,s*np.sqrt(Delta)
			logprob = -np.log(np.sqrt(2*np.pi)*np.abs(b0))-(x1-a0)**2/(2*b0**2)
			loglk = np.mean(logprob)
		elif self.dim==2:
			if name=='MdOU':
				Mean = np.array(x0)@np.array(self.eqn_config.mu)*Delta

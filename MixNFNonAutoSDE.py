...
class NFSSDE(nn.Module):
	...
	def train(self, train_data, para_data, model_path, hist_path, Monitor, DatVes, predt_path):
		...
		for epoch in range(self.n_epochs):
			for batch,train_x,para_x in tqdm.tqdm(zip(np.arange(N_batch)+1,train_dataset,para_dataset), total=N_batch):
				...
				est_drift = self.Model_drift.myevaluate(train_x[:,:self.dim],para_x[:,:self.dim_para])
				z, prior_logprob, log_det = self.forward(train_x[:,:self.dim],para_x[:,:self.dim_para],train_x[:,self.dim:]-est_drift)
	...
	def predict(self,x0,para):
		...
		return self.Model_drift.myevaluate(x0,para)+re
...
class Monitor(NFNonAutoSDE.Monitor):
...
class DataTran(NFNonAutoSDE.DataTran):
	def train_hiddendata(self):
		if self.eqn_config.resmodel=='ResNetwM':
			...
			if self.eqn_config.eqn_name=='DisturbOU':
				self.Model_drift = DisturbOU_exact_drift(self.eqn_config.mu,self.eqn_config.Delta)
		else:
			raise AttributeError('train_hiddendata::No this model')

class DisturbOU_exact_drift:
	def __init__(self,mu,Delta):
		self.mu = mu
		self.Delta = Delta
		self.handle = torch.tensor((1,self.Delta,self.Delta**2))
	def myevaluate(self,x,para):
		return x+self.Delta*(-self.mu*x+self.para_trans(para))
	def myevaluateincrem(self,x,para):
		return self.Delta*(-self.mu*x+self.para_trans(para))
	def para_trans(self,para):
		return torch.sum(self.handle*para,axis=-1,keepdim=True)

...
class NFSSDE(nn.Module):
	...
	def train(self, train_data, model_path, hist_path, Monitor, DatVes, predt_path):
		...
		## train data
		est_drift  = self.drift_data_transfer(self.Model_drift.myevaluate(train_data[:,:self.dim]))
		est_drift  = torch.from_numpy(est_drift).to(torch.float32)
	...
	def postprocs_choose(self):
		if self.eqn_config.eqn_name=='SSAVilar2002R':
			...
			self.drift_data_transfer = self.SSAVilar2002R_drift_data_transfer
	...
	def SSAVilar2002R_predict_discrete(self,x0):
		...
		Dx0   = torch.from_numpy(self.Model_drift.myevaluate(x0)).to(torch.float32)
		re = Dx0[:,[0,1,4,5,6,7,8]]+re
	...
	def SSAVilar2002R_drift_data_transfer(self,data):
		return data[:,np.array([0,1,4,5,6,7,8])]
...
class Monitor(MixNFSDE.Monitor):
	pass

class DataTran(MixNFSDE.DataTran):
	pass

...
class FCNN(nn.Module):
    ...
    """
    def __init__(self, in_dim, out_dim, nodes=8, layer=3, act='tanh'):
        super().__init__()
        if act=='tanh':
            Actfun = nn.Tanh
        elif act=='Relu':
            ...
            raise AttributeError('No this activation function')
        self.network = nn.ModuleList()
        self.network += [nn.Linear(in_dim, nodes)]
        self.network += [Actfun()]
        for i in range(layer-2):
            self.network += [nn.Linear(nodes, nodes)]
            self.network += [Actfun()]
        self.network += [nn.Linear(nodes, out_dim)]
...
class DANN(nn.Module):
    """
    Dissambly-Assembly NN

    Chen, Z., Churchill, V., Wu, K., & Xiu, D. (2022). 
    ...
    """
    def __init__(self, in_dim, out_dim, n_d, n_a, n_w, J, act='tanh'):
        super().__init__()
        self.J = J
        self.n_a = n_a
        self.n_d = n_d
        self.n_w = n_w
        self.Disassembly_block=[FCNN(in_dim, self.n_w, self.n_w, self.n_d, act) for _ in range(self.J)]
        self.Assembly_layer = FCNN(self.J, 1, self.J, self.n_a, act)
        self.out_layer = nn.Linear(self.n_w, out_dim)

    def forward(self, x):
        D = torch.zeros([x.shape[0],self.n_w,self.J])
        for i in range(self.J):
            D[:,:,i] = self.Disassembly_block[i](x)
        A = torch.zeros([x.shape[0],self.n_w])
        for i in range(self.n_w):
            A[:,[i]] = self.Assembly_layer(D[:,i,:])
        z = self.out_layer(A)
        return z


class MAFCond(nn.Module):
    """
    Masked auto-regressive flow.

    [Papamakarios et al. 2018]
    """
    def __init__(self, dim, base_network=FCNN, config=None):
        super().__init__()
        self.dim = dim
        self.layers = nn.ModuleList()
        self.layers += [base_network(dim, 2, config.net_spec["nodes"], config.net_spec["layer"], config.net_spec["act"])]
        for i in range(1, dim):
            self.layers += [base_network(i+dim, 2, config.net_spec["nodes"], config.net_spec["layer"], config.net_spec["act"])]
    ...
    def forward(self, x0, x):
        ...
        for i in range(self.dim):
            if i == 0:
                ...
                out = self.layers[i](torch.cat((x0,x[:, :i]),1))
                mu, alpha = out[:, 0], out[:, 1]
...
class MAFCond_DANN(nn.Module):
    """
    Masked auto-regressive flow.

    [Papamakarios et al. 2018]
    """
    def __init__(self, dim, n_d=1, n_a=1, n_w=20, J=5, base_network=FCNN, config=None):
        super().__init__()
        self.dim = dim
        self.layers = nn.ModuleList()
        self.layers += [base_network(dim, 2, config.net_spec["nodes"], config.net_spec["layer"], config.net_spec["act"])]
        for i in range(1, dim):
            self.layers += [base_network(i+dim, 2, config.net_spec["nodes"], config.net_spec["layer"], config.net_spec["act"])]
        self.DAprior = DANN(dim, dim, n_d, n_a, dim, J)
    ...
    def forward(self, x0, x):
        ...
        for i in range(self.dim):
            if i == 0:
                out = self.layers[0](Dx0)
                mu, alpha = out[:, 0], out[:, 1]
            else:
                out = self.layers[i](torch.cat((Dx0,x[:, :i]),1))
                mu, alpha = out[:, 0], out[:, 1]
            z[:, i] = (x[:, i] - mu) / torch.exp(alpha)
            log_det -= alpha
        return z.flip(dims=(1,)), log_det
...
class NSF_ARCond(nn.Module):
    """
    Neural spline flow, auto-regressive.

    [Durkan et al. 2019]
    """
    def __init__(self, dim, K = 20, B = 15, base_network = FCNN, config=None):
        super().__init__()
        self.dim = dim
        self.K = K
        self.B = B
        self.layers = nn.ModuleList()
        self.layers += [base_network(dim, 3 * K - 1, config.net_spec["nodes"], config.net_spec["layer"], config.net_spec["act"])]
        for i in range(1, dim):
            self.layers += [base_network(i+dim, 3 * K - 1, config.net_spec["nodes"], config.net_spec["layer"], config.net_spec["act"])]

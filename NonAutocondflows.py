...
class MAFCond(nn.Module):
    ...
    """
    def __init__(self, dim, dim_para, base_network=FCNN, config=None):
        super().__init__()
        self.dim = dim
        self.layers = nn.ModuleList()
        self.layers += [base_network(dim+dim_para, 2, config.net_spec["nodes"], config.net_spec["layer"], config.net_spec["act"])]
        for i in range(1, dim):
            self.layers += [base_network(i+dim+dim_para, 2, config.net_spec["nodes"], config.net_spec["layer"], config.net_spec["act"])]
    ...
    def forward(self, x0, para, x):
        ...
        for i in range(self.dim):
            if i == 0:
                out = self.layers[0](torch.cat((x0,para),1))
                mu, alpha = out[:, 0], out[:, 1]
            else:
                out = self.layers[i](torch.cat((x0,x[:, :i],para),1))
                mu, alpha = out[:, 0], out[:, 1]

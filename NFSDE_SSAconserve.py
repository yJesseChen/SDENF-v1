...
class NFSSDE(nn.Module):
	...
	def build_model(self):
		...
		flows = [flowmodel(dimx0=self.dim,dimx=self.dim-1,config=self.net_config) for _ in range(self.flevel)]
		self.prior = torch.distributions.MultivariateNormal(torch.zeros(self.dim-1), torch.eye(self.dim-1))
	...
	def train_data_transfer(self,data):
